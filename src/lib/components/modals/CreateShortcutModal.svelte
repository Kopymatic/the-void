...
<BaseModal hideWhenUnfocused={false} bind:showModal>
	...
	>
		<div class="border-primary bg-secondary box-border max-h-min w-full rounded-md border-2 px-4">
			<div class="mx-auto inline-block py-4">
				<input
					id="autoGenerated"
					type="checkbox"
					name="autoGenerated"
					class="text-accent focus:ring-accent h-6 w-6 rounded-sm"
					bind:checked={auto}
				/>
				<label for="unlisted" class="ms-2 w-full py-4 text-nowrap">Auto Generate Name?</label>
			</div>
		</div>
		...
		<div class="mt-5 flex flex-auto gap-2">
			<PrimaryButton type="submit" icon="check" class="w-full" text="Create Shortcut" />
			<SecondaryButton
				icon="cancel"
				text="Cancel"
				class="w-full"
