...
<ContentBox prose class="mx-auto place-content-between duration-50">
	<div class="flex justify-between">
		<span class="overflow-hidden text-lg text-ellipsis">
			{`${name}`}
		</span>
		<span class="group flex flex-nowrap gap-1">
			<IconButton onclick={openShareMenu} icon="share" class="" />
			...
			{#if !shortcut.autoGenerated}
				<IconButton onclick={() => (showEditModal = true)} icon="edit" class="" />
			{/if}
			<IconButton onclick={() => (showDeleteModal = true)} icon="trash" class="" />
		</span>
	</div>
