...
<form
	...
	class="hover-shrink-div group generic-border-children flex w-full flex-col *:mb-3 *:w-full"
...
>
	<div class="p-3">
		<p class="mb-2 text-xl font-bold">Where do you want your post?</p>
		...
		{#if selectedCategory === "custom"}
			...
			{/if}
		{/if}
		<label class="" for="url"> Desired URL for the post </label>
		...
		{#if error === CreateFormError.invalidUrl || error === CreateFormError.missingUrl}
			<p class="error">{error}</p>
		{/if}
		<p class="my-4 text-xl font-bold">Your post will be at {completeUrl}</p>
	</div>
	<div class="p-3">
		<p class="mb-2 text-xl font-bold">Write your post.</p>
		...
		<textarea class="h-64 w-full" id="body" name="body" required={true}></textarea>
		{#if error === CreateFormError.missingBody || error === CreateFormError.missingUrl}
			<p class="error">{error}</p>
		{/if}
		<div
			class="border-primary bg-secondary mt-2 box-border flex max-h-min w-full items-center rounded-md border-2 px-4"
		>
			<input
				id="unlisted"
				type="checkbox"
